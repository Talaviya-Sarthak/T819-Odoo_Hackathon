...
export default function Negotiation() {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessageInput('');
      const updatedThread = await negotiationsApi.getMessages(activeQuotationId);
  ...
  const handleSubmitCounterDiscount = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const result = await negotiationsApi.requestChange(activeQuotationId, {
        requestedDiscountPercent: discount,
        notes: counterNotes,
      });
      toast.success(
        result.message || `Counter-discount of ${discount}% submitted for review.`,
        'Counter-Offer Sent'
      );
      setCounterDiscount('');
      setCounterNotes('');
            ...
            <div className="rounded-xl border border-purple-500/30 bg-purple-500/10 p-5 shadow-xs space-y-4">
              ...
              <form onSubmit={handleSubmitCounterDiscount} className="space-y-3">
                ...
                <button
                  ...
                  className="w-full rounded-lg bg-primary p-2.5 text-xs font-bold text-primary-foreground shadow-xs hover:bg-primary/90 disabled:opacity-50 transition-colors"
                >
